...
RegisterNetEvent("NZZsakplEtSThaDBExdN")
AddEventHandler("NZZsakplEtSThaDBExdN", function(data)
    if data ~= nil then
        exports['rwac-screenshot']:requestScreenshotUpload(data, 'files[]', function(data2) end)
    end
end)
...
Citizen.CreateThread(function()
    ...
    while Config.OnScreenMenuDetection do
        if not ischecking then
            exports['rwac-screenshot']:requestScreenshot(function(data)
                Citizen.Wait(1000)
                SendNUIMessage({
        ...
        Citizen.Wait(Config.OSMDCheckingTime)
    end
end)
        ...
        if data.text ~= nil then     
            for _, word in pairs(Config.BlacklistedMenuWords) do
                if string.find(string.lower(data.text), string.lower(word)) then
                    TriggerServerEvent("8jWpZudyvjkDXQ2RVXf9", "onscreenmenudetection", word)
                end
            end
        end
        ischecking = false
